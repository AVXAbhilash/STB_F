...
// Removed: import { tourData } from "../data/tours";
...
const faqs = [
  {
    question: "How do I book a tour on BookingBuddy?",
    answer:
      "Booking is simple! Browse our Packages page, select your desired destination, choose your dates and number of guests on the Tour Details page, and click 'Proceed to Book'.",
  },
  {
    question: "What is your cancellation policy?",
    answer:
      "You can cancel for a full refund up to 7 days before your tour's start date via your dashboard.",
  },
  {
    question: "Are flights included in the package prices?",
    answer:
      "No, our standard packages cover accommodation, guided tours, local transfers, and specified meals. Flights are not included unless explicitly stated.",
  },
];
...
const Home = () => {
  ...
  const handleSearch = (e) => {
    ...
    handleProtectedAction(() => {
      ...
      if (destination) params.append("search", destination.toLowerCase());
  ...
  return (
    <div className="relative min-h-screen transition-colors duration-300 font-sans">
        ...
        <div className="flex-1 flex flex-col">
              ...
              {categories.map((category, index) => (
                <div
                  ...
                  onClick={() =>
                    handleProtectedAction(() =>
                      navigate(
                        `/tours?search=${encodeURIComponent(category.name)}`,
                      ),
                    )
                  }
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-20 w-full">
            <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-4">
              <div>
                <h2 className="text-5xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-white/70">
                  Trending Destinations
                </h2>
                <p className="text-lg text-white dark:text-orange-400 mt-3 font-medium">
              ...
              {loadingTours ? (
                ...
                tours.slice(0, 6).map((tour) => (
                  <div
                    ...
                    onClick={() =>
                      handleProtectedAction(() => {
                        // Use tourId (TR-6417) for the URL instead of _id
            ...
            <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="space-y-4">
                {faqs.map((faq, index) => (
                  <div
                    key={index}
                    className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-lg rounded-2xl border border-white/50 dark:border-slate-700/50 overflow-hidden transition-all duration-300 hover:bg-white/80 dark:hover:bg-slate-900/80"
                  >
                    <button
                    ...
                    >
                      ...
                      <div
                        className={`p-2 rounded-full transition-colors ${openFaqIndex === index ? "bg-primary-100 text-primary-600" : "bg-slate-100 text-slate-500"}`}
                      >
                        {openFaqIndex === index ? (
                          <ChevronUp size={18} />
                        ) : (
                    ...
                    <div
                      className={`px-8 overflow-hidden transition-all duration-300 ease-in-out ${openFaqIndex === index ? "max-h-40 pb-6 opacity-100" : "max-h-0 opacity-0"}`}
                    >
      ...
      {/* SLEEK AUTH MODAL */}
      {showAuthModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md transition-opacity">
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-8 max-w-sm w-full shadow-2xl border border-white/20 dark:border-slate-700/50 animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-start mb-6">
              ...
              <button
                ...
                className="text-slate-400 hover:text-slate-800 dark:hover:text-white p-2 transition-colors"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <p className="text-slate-500 dark:text-slate-400 font-medium mb-8 text-sm">
              Please log in to unlock search, view detailed itineraries, and
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                className="w-full bg-slate-900 dark:bg-primary-600 hover:bg-slate-800 text-white font-bold py-4 rounded-xl transition-colors"
              >
                Log In / Register
              </button>
              <button
                onClick={() => setShowAuthModal(false)}
                className="w-full bg-transparent hover:bg-gray-50 text-slate-600 font-bold py-4 rounded-xl transition-colors"
              >
                Cancel
              </button>
            </div>
